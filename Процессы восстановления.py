...
df_volumes = df_volumes[df_volumes['date_only'].dt.weekday < 5]


df_volumes = df_volumes.sort_values('end')


df_volumes['cumulative_recovery'] = df_volumes.groupby('date_only')['volume'].cumsum()
df_volumes.to_excel('1.xlsx')

plt.figure(figsize=(14, 8))


dates = df_volumes['date_only'].unique()
colors = plt.cm.tab10(np.linspace(0, 1, len(dates)))  

for date, color in zip(dates, colors):
    day_data = df_volumes[df_volumes['date_only'] == date]

    time_numeric = day_data['end'].dt.hour + day_data['end'].dt.minute / 60 + day_data['end'].dt.second / 3600
    
    plt.plot(time_numeric, 
             day_data['cumulative_recovery'], 
             marker='o', 
             linewidth=2, 
             markersize=3, 
             color=color,
...
plt.title('Процесс восстановления - кумулятивная сумма объема внутри дня по BR за сентябрь')
plt.xlabel('Время (часы)')
plt.ylabel('Накопленная сумма объема')
